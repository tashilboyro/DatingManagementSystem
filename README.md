...
This project uses MSTest as the unit testing framework.
...
1.Install MSTest packages (if not already installed):
...
2. Before initializing the project find the `DatingManagementSystemTests` folder within `DatingManagementSystem` folder.

 

3. Extract `DatingManagementSystemTests` folder from `DatingManagementSystem` and place both folders in the same directory in your local file system

   i.e cut and paste `DatingManagementSystemTests` folder in the same folder as `DatingManagementSystem`.

 

4. On your Visual Studio, Right-Click on the `DatingManagementSystemTests` solution --> Add --> Project Reference --> Check DatingManagementSystem box --> OK.

 

5. The main project can then be run provided required packages have been installed.

 

### Folder Structure with Testing
...
/DatingManagementSystem.Tests/

    CompatibilityScoreTests.cs

    JaccardSimilarityTests.cs

    RemoveAlgorithmTests.cs

    SortedCompatibilityScoresTests.cs
...
# Accessing Folders on GIT Repository

 

The following 3 folders are present in the repository where you will find all necessary documents:

 

1. CSV & SQL Database Code
